...
Storage.prototype.storeEvent = function(event, callback) {
  ...
  var transaction = self.edb.multi();
  transaction.rpush( 'aggregate:' + event.aggregateId + ':events', data );
  transaction.rpush( 'events:' + event.name, data );
  transaction.exec(function(err, index) {
    if(callback) {
      callback();
    }
  });
